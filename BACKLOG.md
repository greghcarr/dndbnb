...
- **Starter-pack content coverage.** Engine-side accounting of what's in the pack per category (and which spells / features ship schema-only because of missing primitives) lives in the sibling engine repo at [../dnd-srd-engine/docs/starter-pack-gaps.md](../dnd-srd-engine/docs/starter-pack-gaps.md). The asks above are the primitives blocking dndbnb features; the gaps doc names which content currently waits on each one.
